<template>
    <div class="calendar">
            ...
            <div class="calendar-info" @click.stop="changeYear">
                ...
                <div class="month">
                    <div class="month-inner" :style="{'top':-(this.month*20)+'px'}">
                        <span v-for="(m, index) in months" v-bind:key="index">{{m}}</span>
                    </div>
                </div>
        ...
        <thead>
            <tr>
                <td v-for="(week, index) in weeks" class="week" v-bind:key="index">{{week}}</td>
            </tr>
        </thead>
        <tbody>
        <tr v-for="(day,k1) in days" v-bind:key="k1" style="{'animation-delay',(k1*30)+'ms'}">
            <td v-for="(child,k2) in day" v-bind:key="k2" :class="{'selected':child.selected,'disabled':child.disabled}" @click="select(k1,k2,$event)">
        ...
        <div class="calendar-years" :class="{'show':yearsShow}">
            <span v-for="(y,index) in years"  v-bind:key="index" @click.stop="selectYear(y)" :class="{'active':y==year}">{{y}}</span>
        </div>
